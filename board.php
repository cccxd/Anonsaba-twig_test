...
if ($_POST['MAX_FILE_SIZE'] != $board_class->board['maximagesize']) {
	exitWithErrorPage(_gettext('You look like a bot.'));
}
...
if ($posting_class->CheckValidPost($is_oekaki)) {
	...
	$rate = $tc_db->GetOne("SELECT HIGH_PRIORITY count(id) FROM `" . KU_DBPREFIX . "posts` WHERE `boardname` = '" . $_POST['board'] . "' AND `parentid` = 0 AND `timestamp` >= " . (time() - 60));
	if (!$post_isreply && $rate >= 5) {
		$recaptchaenabled = $tc_db->GetOne('SELECT `enablecaptcha` FROM `'.KU_DBPREFIX.'boards` WHERE `name` = "'.$_POST['board'].'"');
		if ($recaptchaenabled == 0) {
			$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'boards` SET `enablecaptcha` = 1 WHERE `name` = "'.$_POST['board'].'"');
			$board_class = new Board($_POST['board']);
			$board_class->RegenerateAll();
			management_addlogentry('Spam detected on /'. $_POST['board'].'/', 1, 'Anonsaba');
			exitWithErrorPage(_gettext('You look like a bot.'));
		}
	}
	if ($post_isreply) {
