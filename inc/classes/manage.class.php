...
class Manage {
	...
	function email() {
		...
		if (isset($_GET['act'])) {
			if ($_GET['act'] == 'add') {
				if (!in_array($_POST['staffname'], $users)) {
					exitWithErrorPage(_gettext('There is no username by that name. Please enter a valid username for '.$_POST['email'].'.'));
				}
				$tc_db->Execute('INSERT INTO `'.KU_DBPREFIX.'email` (`email`, `staffname`, `postemail`, `reportemail`) VALUES ("'.$_POST['email'].'", "'.$_POST['staffname'].'", '.$postemail.', '.$reportemail.')');
				$tpl_page .= '<hr /><h3>'. _gettext($_POST['email'].' has been added.').'</h3><hr /><br />';
			} elseif ($_GET['act'] == 'del') {
				$tc_db->Execute('DELETE FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['id']));
				$tpl_page .= '<hr /><h3>'. _gettext('Email has been deleted.') .'</h3><hr />';
			} elseif ($_GET['act'] == 'edit') {
				$action = 'edit&amp;id='. $_GET['id'];
				$results = $tc_db->GetAll('SELECT * FROM `'.KU_DBPREFIX.'email` WHERE `id` = '.$tc_db->qstr($_GET['id']));
				$values = $results[0];
				if ($values['postemail'] == 1) {
					$check = 'checked="checked"';
				} else {
					$check = '';
				} 
				if ($values['reportemail'] == 1) {
					$check1 = 'checked="checked"';
				} else {
					$check1 = '';
				}
				if (isset($_POST['submit'])) {
					$tc_db->Execute('UPDATE `'.KU_DBPREFIX.'email` SET `email` = "'.$_POST['email'].'", `staffname` = "'.$_POST['staffname'].'", `postemail` = '.$postemail.', `reportemail` = '.$reportemail.' WHERE `id` = '.$tc_db->qstr($_GET['id']));
					$tpl_page .= '<hr /><h3>'. _gettext('Email succesfully edited.') .'</h3><hr />';
				}
			}
		}
		$tpl_page .= '<form action="manage_page.php?action=email&amp;act='. $action . '" method="post">
				<label for="email">'. _gettext('Email') . ':</label>
				<input type="text" id="email" name="email" value="'. (isset($values['email']) ? $values['email'] : '') . '" /><br />
				<label for="staffname">'. _gettext('Staff Username') .':</label>
				<input type="text" id="staffname" name="staffname" value="'. (isset($values['staffname']) ? $values['staffname'] : '') . '" />
				<div class="desc"><b>'. $final . '</b></div><br />
				<label for="postemail">'. _gettext('Send Email for every post') .':</label>
				<input type="checkbox" name="postemail" id="postemail" '.(isset($check) ? $check : '').' /><br />
				<label for="reportemail">'. _gettext('Send Email for every report') .':</label>
				<input type="checkbox" name="reportemail" id="reportemail" '.(isset($check1) ? $check1 : '').' />
				<br />
		...
		if (count($query) > 0) {
			...
			foreach ($query as $line) {
				...
				$tpl_page .= '<tr><td>'. $line['email'] . '</td><td>'. $line['staffname'] . '</td><td>'. $answer . '</td><td>'. $answer1 . '</td><td>[<a href="?action=email&amp;act=edit&amp;id='. $line['id'] . '">'. _gettext('Edit') .'</a>]&nbsp;[<a href="?action=email&amp;act=del&amp;id='. $line['id'] . '">'. _gettext('Delete') .'</a>]</td></tr>';
			}
			$tpl_page .= '</table>';
		}
	}
	...
	function ads() {
		...
		if (isset($_GET['edit'])) {
			if (isset($_POST['code']) and !empty($_POST['code'])) {
				...
				$tc_db->Execute("UPDATE `" . KU_DBPREFIX . "ads` SET `disp` = " . $displayed . ", `code` = " . $tc_db->qstr($_POST['code']) . " WHERE `id` = " . $tc_db->qstr($_GET['edit']) . "");
			...
			foreach ($results as $ad) {
				...
				'<input type="checkbox" name="disp" '.$checked.'  /><div class="desc">'. _gettext('Check to display.') .'</div><br />'. "\n" .
				'<label for="code">'. _gettext('Code') .':</label>'. "\n" .
				'<textarea name="code" rows="10" cols="25">' . htmlspecialchars($ad['code']) . '</textarea><br />' . "\n" .
	...
	function ipsearch() {
		...
		$resultssearch = $tc_db->GetAll("SELECT * FROM `" . KU_DBPREFIX . "posts`  WHERE ".$queryextra. " ipmd5 = '" . md5(KU_SALT.$_GET['ip'])."'");
	...
	function bans() {
		...
		if (isset($_POST['modban']) && is_array($_POST['post']) && $_POST['board']) {
			...
			if (!empty($ban_board_id)) {
				foreach ( $_POST['post'] as $post ) {
					...
					$results1 = $tc_db->GetAll("SELECT HIGH_PRIORITY * FROM `" . KU_DBPREFIX . "post_files` WHERE `boardid` = '" . $ban_board_id . "' AND `id` = " . intval($post) . "");
					if (count($results) > 0 || count($results1) > 0) {
						$multiban[] = md5_decrypt($results[0]['ip'], KU_RANDOMSEED);
						$multiban_hash[] = $results1[0]['file_md5'];
						$multiban_parentid[] = $results[0]['parentid'];
					}
				}
			}
		}
		$hashfinaltime = time() + $_POST['banhashtime'];
		if (!$_POST['bancheckip'] && $_POST['filecheck'] &&  $_POST['addban'] && !isset($_POST['multibanhashes'])) {
			$banhash = $_POST['hash'];
			$tc_db->Execute("INSERT INTO `".KU_DBPREFIX."bannedhashes` ( `md5` , `bantime` , `description`, `addedby`, `dateadded` ) VALUES ( ".$tc_db->qstr($banhash)." , ".$hashfinaltime." , ".$tc_db->qstr($_POST['banhashdesc'])." , '".$_SESSION['manageusername']."' , " .time(). ")");
		...
		} elseif (isset($_POST['banhashtime']) && $_POST['filecheck'] && $_POST['banhashtime'] != '' && ($_POST['hash'] != '' || isset($_POST['multibanhashes'])) && $_POST['banhashtime'] >= 0) {
			if ($_POST['multibanhashes'] != '')
		...
		if (((isset($_POST['ip']) || isset($_POST['multiban'])) && isset($_POST['seconds']) && $_POST['checkbanip'] && (!empty($_POST['ip']) || (empty($_POST['ip']) && !empty($_POST['multiban'])))) || $instantban) {
			if ($_POST['seconds'] >= 0 || $instantban) {
				...
				foreach ($ban_ips as $ban_ip) {
					...
					if ($ban_type == 2) {
						$bannedtype = 'Whitelisted';
					} else {
						$bannedtype = 'Banned';
					}
					$logentry = $bannedtype . ' '. $ban_ip;
					$logentry .= ($ban_duration == 0) ? ' '. _gettext('without expiration') : ' '. _gettext('until') . ' '. date('F j, Y, g:i a', time() + $ban_duration);
					$logentry .= ' - '. _gettext('Reason') . ': '. $ban_reason . (($ban_note) ? (" (".$ban_note.")") : ("")). ' - '.$bannedtype.''. _gettext(' from') . ': ';
		...
		if (!$multiban) {
			...
			$tpl_page .= '<input type="hidden" name="multiban" value="'.htmlspecialchars(serialize($multiban)).'">Multiple IPs
		...
		if ($multiban) {
			$tpl_page .=' <input type="hidden" name="multibanhashes" value="'.htmlspecialchars(serialize($multiban_hash)).'">Multiple Hashes';
		} else {
			$tpl_page .= '<input type="text" name="hash" value="'. $ban_hash . '" />';
		}
		$tpl_page .= '<div class="desc">'. _gettext('The hash of the File you are trying to ban.') . '</div><br />
