...
if ($_GET['board']) {
	...
	$boardname = $results[0]['name'];
}
if (isset($_GET['do'])) {
	switch($_GET['do']) {
	case 'addthread':
		$threadexist = $tc_db->GetOne("SELECT COUNT(*) FROM `".KU_DBPREFIX."posts` WHERE `id` = ".$tc_db->qstr($_GET['threadid'])." AND `boardname` = ".$tc_db->qstr($_GET['board']));
	...
	$newestreplyid = $tc_db->GetOne("SELECT `id` FROM `".KU_DBPREFIX."posts` WHERE `boardname` = ".$tc_db->qstr($_GET['board'])." AND `IS_DELETED` = 0 AND `parentid` = ".$tc_db->qstr($_GET['threadid'])." ORDER BY `id` DESC LIMIT 1");
	...
	if (!$cached) {
		$watched_threads = $tc_db->GetAll("SELECT `threadid` , `lastsawreplyid` FROM `watchedthreads`   WHERE `ip` = '" . $_SERVER['REMOTE_ADDR']."'  AND `board` = ". $tc_db->qstr($_GET['board']) ."   ORDER BY `lastsawreplyid` DESC"      ); 
